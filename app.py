...
with st.expander("⚙️ Reset Totale Sessione"):
    if st.button("⚠️ RESETTA TUTTA L'ASTA"):
        ...
        st.session_state.storico = []
        st.rerun()# 8. CHEAT SHEET DINAMICO AUTOMATIZZATO
st.divider()
st.subheader("🎯 Target Giocatori Rimanenti (Si aggiorna da solo)")

TARGET_GIOCATORI = {
    "🧤 PORTIERI": ["Maignan", "Martínez", "Meret", "Svilar", "Carnesecchi"],
    "🛡️ DIFENSORI": ["Dimarco", "Bremer", "Wesley", "Akanji", "Solet", "Mancini", "Di Lorenzo"],
    "⚙️ CENTROCAMPISTI": ["Calhanoglu", "McTominay", "Nico Paz", "Orsolini", "Pulisic", "Frattesi"],
    "⚽ ATTACCANTI": ["Lautaro", "Malen", "Ramos", "Hojlund", "Yildiz", "Dovbyk", "Davis"]
}

# Ricava la lista dei giocatori già acquistati nello storico
presi = [item["Giocatore"].lower().strip() for item in st.session_state.storico]

cols = st.columns(4)
for i, (cat, lista) in enumerate(TARGET_GIOCATORI.items()):
    with cols[i % 4]:
        st.write(f"**{cat}**")
        for g in lista:
            # Controlla se il nome del giocatore è già stato registrato
            gia_preso = any(g.lower() in p for p in presi)
            if gia_preso:
