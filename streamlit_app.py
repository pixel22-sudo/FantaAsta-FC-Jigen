# VERSIONE v3.11 SEMAFORO ASTA - GERARCHIE + SCOMMESSE + ASSISTENTE
...
PLAYER_INTEL.update({
    ...
    "Kean":{
        "verdict":"🟢 TITOLARE","titolarita":"ALTA","risk":"BASSO",
        "summary":"È il centravanti titolare della Fiorentina.",
        "competition":"Parte nell'undici base.",
        "setpieces":"Secondo rigorista indicato dopo Gudmundsson.",
        "auction":"Buon secondo/terzo slot premium se il prezzo resta sotto i top assoluti.",
        "tag":"TITOLARE","source":"Fantacalcio • gerarchie 2026/27"
    },
    "Orsolini":{
    ...
    "Douvikas":{
        "verdict":"🟢 TITOLARE + RIGORI","titolarita":"ALTA","risk":"BASSO",
        "summary":"È il centravanti indicato nell'undici base del Como.",
        "competition":"Parte titolare.",
        "setpieces":"Secondo rigorista indicato dopo Da Cunha.",
        "auction":"Profilo molto interessante per rapporto prezzo/titolarità.",
        "tag":"VALUE + RIGORI","source":"Fantacalcio • gerarchie 2026/27"
    },
    "Krstovic":{
        "verdict":"🟡 BALLOTTAGGIO","titolarita":"MEDIA","risk":"MEDIO",
...
def auction_signal(row, current_price, plan, info):
    ...
    remaining = int(S.get("budget", 1000) or 0)
    role = str(row.Ruolo)
    role_max = {"POR":3, "DIF":8, "CEN":8, "ATT":6}.get(role, 0)
    owned_role = sum(1 for x in S.get("roster", []) if x.get("Ruolo") == role)
...
st.markdown(
    f"""<div class="jigen-head">
      <div class="jigen-brand">⚽ FC Jigen • v3.9 SCOMMESSE + ASSISTENTE</div>
...
tabs=st.tabs(["🔥 LIVE","🧠 MASTER","📡 RADAR","👥 RIVALI","📋 ROSA","🎯 PIANO","🎲 SCOMMESSE","📈 STORICO"])
...
with tabs[0]:
    ...
    if choice:
        ...
        with st.expander("🧠 ASSISTENTE ASTA", expanded=True):
