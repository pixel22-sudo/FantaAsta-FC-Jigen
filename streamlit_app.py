# VERSIONE v3.13 DEADLINE AUDIT - MERCATO + STORICO SICURO
...
def rebuild_from_moves(moves):
    """Ricostruisce lo stato asta usando lo schema reale dell'app."""
    S["credits"] = BUDGET
    S["roster"] = []
    S["out"] = []
    S["moves"] = []
    S["rivals"] = {n: {"credits": BUDGET, "slots": 25, "roles": dict(SLOTS)} for n in RIVALS}
    ...
    for mv0 in moves:
        ...
        try:
            ...
            # Changing buyer in Storico changes the action coherently.
            if buyer == "FC Jigen":
                action = "MIO"
                if price <= 0 or price > S["credits"]:
                    errors.append(f"{name}: prezzo non valido")
                    continue
                if sum(1 for x in S["roster"] if x.get("role")==role) >= SLOTS.get(role,0):
                ...
                if price <= 0:
                    errors.append(f"{name}: prezzo non valido")
                    continue
                if buyer in S["rivals"]:
                    d = S["rivals"][buyer]
                    if d["slots"] <= 0 or d["roles"].get(role,0) <= 0 or price > d["credits"]:
                        errors.append(f"{name}: stato rivale incompatibile")
                        continue
                    d["credits"] -= price
                    d["slots"] -= 1
                    d["roles"][role] -= 1
    ...
    if moves_hist:
        ...
        with hc1:
            if not is_unsold and st.button("💾 SALVA MODIFICA",use_container_width=True,key="history_save_edit"):
                ...
                persist()
                if errs:
                    st.error("Controlla la modifica: "+" | ".join(errs[:3]))
                else:
                    st.success("✅ Movimento modificato.")
                st.rerun()
        ...
        with hc2:
            if st.button("🗑️ ELIMINA MOVIMENTO",use_container_width=True,key="history_delete_move"):
                ...
                persist()
                if errs:
                    st.error("Eliminato con avvisi: "+" | ".join(errs[:3]))
                else:
