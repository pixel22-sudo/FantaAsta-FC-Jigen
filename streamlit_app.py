# VERSIONE v3.21.1 FANTAMOSSA - FIX BOTTOM NAV OVERLAY
...
st.set_page_config(page_title="FantaMossa • FC Jigen", page_icon=FANTAMOSSA_ICON, layout="wide")
...
.block-container{max-width:100%!important;padding-top:.55rem;padding-bottom:3.2rem;overflow-x:hidden!important}
...
@st.fragment
def render_mossa():
    fm_page("🧠 La Mossa", "FantaMossa trasforma lo stato reale dell’asta nella prossima decisione migliore.")

    mossa_tool = st.segmented_control(
        "Strumento",
        ["🎯 Compra", "⚖️ Confronta", "🧮 Simula"],
        default="🎯 Compra",
        key="mossa_tool"
    )

    if mossa_tool == "🎯 Compra":
        ...
        if recs.empty:
            ...
            top = recs.iloc[0]
            st.markdown(
                f'<div class="fm-pick"><div class="fm-pick-kicker">🔥 Mossa consigliata • Score {int(top["Score"])}/100</div>'
...
# v3.21.1 — Navigazione mobile sicura: pagina prima, menu sticky dopo.
...
if main_nav == "🔥 Asta":
    ...
    fm_page("＋ Altri strumenti", "Approfondimenti e gestione avanzata. Durante i rilanci resta su Asta o Mossa.")
...
def _fm_go(page):
    st.session_state["fm_page"] = page

st.markdown("""
<style>
/* v3.21.1 — bottom bar STICKY, non copre il contenuto */
.st-key-fm_bottom_nav{
...
.st-key-fm_bottom_nav [data-testid="stHorizontalBlock"]{
    gap:4px !important;
}
.st-key-fm_bottom_nav [data-testid="column"]{
    min-width:0 !important;
}
.st-key-fm_bottom_nav div.stButton>button{
...
.st-key-fm_bottom_nav div.stButton>button p{
...
.st-key-fm_bottom_nav div.stButton>button[kind="primary"]{
    background:rgba(216,183,95,.18) !important;
    box-shadow:inset 0 -3px 0 #D8B75F !important;
}
.st-key-fm_bottom_nav div.stButton>button[kind="primary"] p{
...
with st.container(key="fm_bottom_nav"):
    n1,n2,n3,n4,n5 = st.columns(5, gap="small")
    with n1:
        st.button("🔥\nAsta", use_container_width=True,
                  type="primary" if main_nav=="🔥 Asta" else "secondary",
                  on_click=_fm_go, args=("🔥 Asta",), key="fm_nav_asta")
    with n2:
        st.button("🧠\nMossa", use_container_width=True,
                  type="primary" if main_nav=="🧠 Mossa" else "secondary",
                  on_click=_fm_go, args=("🧠 Mossa",), key="fm_nav_mossa")
    with n3:
        st.button("👕\nRosa", use_container_width=True,
                  type="primary" if main_nav=="👕 Rosa" else "secondary",
                  on_click=_fm_go, args=("👕 Rosa",), key="fm_nav_rosa")
    with n4:
        st.button("👥\nRivali", use_container_width=True,
                  type="primary" if main_nav=="👥 Rivali" else "secondary",
                  on_click=_fm_go, args=("👥 Rivali",), key="fm_nav_rivali")
    with n5:
        st.button("•••\nAltro", use_container_width=True,
                  type="primary" if main_nav=="＋ Altro" else "secondary",
                  on_click=_fm_go, args=("＋ Altro",), key="fm_nav_altro")
