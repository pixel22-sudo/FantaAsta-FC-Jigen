...
st.title("⚽ FantaAsta FC Jigen • WEB v3.6 iPHONE EASY")
st.caption("Modalità iPhone • LIVE rapido • Target • Cloud • Backup")
...
m2.metric("👕 ROSA",f'{len(S["roster"])}/25')
m3.metric("🔨 MAX",max_absolute())
with st.expander("📊 Dettagli budget / mercato", expanded=False):
    x1,x2=st.columns(2)
    x1.metric("💸 Spesi",BUDGET-S["credits"])
    x2.metric("📊 Mercato",f"{market_index():.2f}x")

tabs=st.tabs(["🔥 LIVE","🧠 MASTER","📡 RADAR","👥 RIVALI","📋 ROSA","🎯 PIANO","📈 STORICO"])

with tabs[0]:
    st.subheader("🔥 ASTA LIVE — modalità rapida")

    qa,qb=st.columns(2)
    qa.button("☁️ SALVA CLOUD",use_container_width=True,on_click=_quick_cloud_save)
    qb.button("↩️ ANNULLA ULTIMA",use_container_width=True,on_click=_quick_undo,
    ...
    if choice:
        ...
        if st.button("✅ REGISTRA ACQUISTO",type="primary",use_container_width=True):
            k=row.key
            if k in S["out"]:
