...
st.title("⚽ FantaAsta FC Jigen • WEB v3 iPHONE")
...
with st.sidebar:
    ...
    if sb_ready:
        if st.session_state.get("_cloud_ok"):
            ...
            st.info("☁️ Cloud configurato")
        if st.button("☁️ SALVA ORA", use_container_width=True):
            st.session_state["_cloud_ok"] = cloud_save(S)
            st.rerun()
    else:
        st.warning("☁️ Cloud non configurato: usa il backup JSON")
    issues=integrity()
    st.success("✅ Stato integro") if not issues else st.error("⚠️ "+", ".join(issues))
