# VERSIONE v3.17 FANTAMOSSA TURBO - IPHONE PERFORMANCE
...
st.set_page_config(page_title="FantaMossa • Asta 2026/27", page_icon=FANTAMOSSA_ICON, layout="wide")
...
st.markdown("""<style>
:root{
  --fm-bg:#001f16; --fm-bg2:#001811; --fm-card:#062d21; --fm-card2:#0a3a2a;
  --fm-green:#0f8861; --fm-green2:#004732; --fm-gold:#b18e41; --fm-gold2:#f7da99;
  --fm-text:#f7f3e8; --fm-muted:#afc4b9; --fm-danger:#d76a6a;
}
html,body,[data-testid="stAppViewContainer"],[data-testid="stMain"]{max-width:100vw!important;overflow-x:hidden!important}
[data-testid="stAppViewContainer"]{
  background:radial-gradient(circle at 50% -10%,#0b4a35 0,#00281d 30%,#001811 76%);
  color:var(--fm-text);
}
[data-testid="stHeader"]{background:#002019}
[data-testid="stSidebar"]{background:linear-gradient(180deg,#00281d,#00140e)!important;border-right:1px solid rgba(177,142,65,.30)}
.block-container{max-width:100%!important;padding-top:.65rem;padding-bottom:3rem;overflow-x:hidden!important}

/* Brand */
.fm-head{display:flex;align-items:center;gap:10px;padding:7px 10px;border:1px solid rgba(177,142,65,.42);border-radius:17px;background:#062d21;margin-bottom:8px}
.fm-logo{width:42px;height:42px;border-radius:11px;flex:0 0 auto}
.fm-brandbox{min-width:0;flex:1}.fm-brand{color:var(--fm-gold2);font-size:1.08rem;font-weight:900;letter-spacing:.01em;line-height:1.05}.fm-sub{color:var(--fm-muted);font-size:.72rem;margin-top:3px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
.fm-cloud{font-size:.70rem;font-weight:800;color:var(--fm-gold2);padding:6px 8px;border:1px solid rgba(177,142,65,.45);border-radius:999px;white-space:nowrap;background:rgba(177,142,65,.08)}
.fm-summary{display:grid;grid-template-columns:1fr 1fr 1fr;gap:7px;margin:5px 0 5px}
.fm-stat{background:#083526;border:1px solid rgba(177,142,65,.28);border-radius:14px;text-align:center;padding:8px 4px}
.fm-stat-label{font-size:.61rem;color:var(--fm-muted);font-weight:800}.fm-stat-value{font-size:1.24rem;color:var(--fm-gold2);font-weight:900;line-height:1.15;margin-top:2px}
.fm-mini{font-size:.69rem;color:var(--fm-muted);text-align:center;margin:0 0 7px}
.fm-step{color:var(--fm-gold2);font-size:.78rem;font-weight:800;letter-spacing:.02em;text-transform:uppercase;margin:.35rem 0 .1rem}

/* Typography */
h1,h2,h3{color:var(--fm-gold2)!important} p,li{color:inherit}
.small,[data-testid="stCaptionContainer"]{color:var(--fm-muted)!important}

/* Inputs / cards */
div[data-testid="stMetric"]{background:rgba(6,45,33,.86);border:1px solid rgba(177,142,65,.28);padding:8px;border-radius:14px}
div[data-testid="stMetricValue"]{color:var(--fm-gold2);font-size:1.27rem}
div[data-baseweb="select"]>div,div[data-testid="stTextInput"] input,div[data-testid="stNumberInput"] input{background:#062d21!important;color:var(--fm-text)!important;border-color:rgba(177,142,65,.45)!important;border-radius:13px!important}
div[data-testid="stTextInput"] input{min-height:47px;font-size:1.03rem}div[data-testid="stNumberInput"] input{font-size:1.35rem;font-weight:900;text-align:center}div[data-testid="stSelectbox"]>div{min-height:47px}

/* Buttons */
div.stButton>button{min-height:48px;font-weight:850;border-radius:13px;border:1px solid rgba(177,142,65,.55);background:#083526;color:var(--fm-gold2)}
div.stButton>button:hover{border-color:var(--fm-gold2);color:#fff;background:#0b4733}
div.stButton>button[kind="primary"]{background:#d8b75f!important;color:#001b13!important;border-color:#f7da99!important;font-weight:950!important}
div.stButton>button[kind="primary"]:hover{background:#e4c66f!important}

/* Segmented navigation */
.st-key-fm_main_nav div[role="radiogroup"]{display:flex!important;width:100%!important;gap:5px!important}
.st-key-fm_main_nav button{flex:1 1 0!important;min-width:0!important;padding:.42rem .18rem!important;font-size:.78rem!important;font-weight:850!important;border-radius:12px!important}
.st-key-fm_main_nav button[aria-checked="true"]{background:#d8b75f!important;color:#001b13!important}

/* Expanders/tables */
[data-testid="stExpander"]{border-color:rgba(177,142,65,.25)!important;border-radius:14px!important;background:rgba(6,45,33,.34)}
[data-testid="stDataFrame"]{border:1px solid rgba(177,142,65,.22);border-radius:12px;overflow:hidden}
hr{border-color:rgba(177,142,65,.25)!important}

@media(max-width:700px){
 .block-container{padding:.55rem .55rem 2rem}.fm-head{padding:6px 8px;border-radius:15px}.fm-logo{width:38px;height:38px}.fm-brand{font-size:1rem}.fm-cloud{font-size:.64rem;padding:5px 6px}.fm-summary{gap:5px}.fm-stat{padding:6px 3px}.fm-stat-label{font-size:.56rem}.fm-stat-value{font-size:1.13rem}
 div.stButton>button{min-height:44px!important;padding:.34rem .38rem!important}div[data-testid="stTextInput"] input,div[data-testid="stSelectbox"]>div{min-height:43px!important}
 .st-key-fm_main_nav button{font-size:.70rem!important;padding:.36rem .08rem!important}
...
st.markdown(
    f"""<div class="fm-head">
      ...
      <div class="fm-brandbox"><div class="fm-brand">FantaMossa</div><div class="fm-sub">FC Jigen • Beta 2026/27</div></div>
...
with st.sidebar:
    st.header("☰ FANTAMOSSA")
    st.caption("Salvataggio, annulla e sicurezza")
...
def render_asta():
    st.markdown("### 🔥 Asta")
    st.caption("Tre passaggi: scegli il giocatore → inserisci il prezzo → registra chi lo prende.")
    ...
    if choice:
        ...
        st.info("Tocca il campo sopra e scrivi il nome: la ricerca avviene direttamente sul telefono.")

@st.fragment
def render_mossa():
    st.subheader("🧠 La Mossa")
    st.caption("Dimmi cosa devi decidere: FantaMossa usa rosa, crediti, slot, piano e prezzi reali dell’asta.")

    mossa_tool = st.segmented_control(
        "Strumento",
        ["🎯 CHI COMPRO", "⚖️ CONFRONTA", "🧮 SIMULA"],
        default="🎯 CHI COMPRO",
        key="mossa_tool"
    )

    if mossa_tool == "🎯 CHI COMPRO":
        m_role = st.segmented_control(
            "Ruolo",
            ["TUTTI","POR","DIF","CEN","ATT"],
            default="TUTTI",
            key="mossa_role_filter"
        )
        recs = mossa_recommendations(m_role, 4)
        if recs.empty:
            st.success("🏆 Nessun acquisto necessario nel filtro selezionato.")
        else:
            top = recs.iloc[0]
            st.success(
                f"🔥 MOSSA: {top['Nome']} • {top['R']} • "
                f"IDEALE {int(top['Ideale'])} • STOP {int(top['STOP'])}"
            )
            st.caption(f"Score FantaMossa {int(top['Score'])}/100 • {top['Perché']}")
            st.dataframe(
                recs[["Nome","R","Squadra","FVM","Score","Ideale","STOP","Stima"]],
                hide_index=True,
                use_container_width=True
            )
            st.info("Il ranking cambia automaticamente mentre registri acquisti, crediti e prezzi degli avversari.")

    elif mossa_tool == "⚖️ CONFRONTA":
        ap_cmp = all_players()
        av_cmp = ap_cmp[~ap_cmp.key.isin(set(S["out"]))].copy()
        labels_cmp = [
            f"{r.Nome} • {r.Ruolo} • {r.Squadra} • FVM {int(r.FVM)}"
            for _, r in av_cmp.sort_values(["FVM","Nome"], ascending=[False,True]).iterrows()
        ]
        if len(labels_cmp) < 2:
            st.info("Servono almeno due giocatori ancora disponibili.")
        else:
            ca = st.selectbox("Giocatore A", labels_cmp, index=None, placeholder="Scegli A...", key="mossa_cmp_a")
            cb = st.selectbox("Giocatore B", labels_cmp, index=None, placeholder="Scegli B...", key="mossa_cmp_b")

            def _cmp_row(label):
                if not label:
                    return None
                name = label.split(" • ")[0]
                mm = av_cmp[av_cmp.Nome.astype(str) == name]
                return mm.iloc[0] if not mm.empty else None
            ...
            if row_a is not None and row_b is not None:
                if str(row_a.key) == str(row_b.key):
                    ...
                    with cpa:
                        pa = st.number_input("Prezzo A (0 = non noto)", min_value=0, step=1, value=0, key="mossa_price_a")
                    with cpb:
                        pb = st.number_input("Prezzo B (0 = non noto)", min_value=0, step=1, value=0, key="mossa_price_b")

                    a, b, winner = mossa_compare(row_a, row_b, pa, pb)
                    if winner == "PARI":
                        st.warning("⚖️ MOSSA: equilibrio quasi perfetto. Decidi sul prezzo reale.")
                    else:
                        st.success(f"🏆 MOSSA CONSIGLIATA: {winner}")

                    cmp_df = pd.DataFrame([a, b]).set_index("Nome").T
                    st.dataframe(cmp_df, use_container_width=True)

                    if int(pa or 0) > 0 or int(pb or 0) > 0:
                        st.caption("Con il prezzo inserito il punteggio include anche PRENDI / VALUTA / LASCIA.")

    else:
        ap_sim = all_players()
        av_sim = ap_sim[~ap_sim.key.isin(set(S["out"]))].copy().sort_values(["FVM","Nome"], ascending=[False,True])
        sim_labels = [f"{r.Nome} • {r.Ruolo} • {r.Squadra} • FVM {int(r.FVM)}" for _,r in av_sim.iterrows()]
        sim_pick = st.selectbox("Giocatore da simulare", sim_labels, index=None, placeholder="Scegli giocatore...", key="mossa_sim_pick")
        if sim_pick:
            sim_name = sim_pick.split(" • ")[0]
            sim_row = av_sim[av_sim.Nome.astype(str) == sim_name].iloc[0]
            sim_plan = effective_target_plan(sim_row)
            sim_default = min(int(sim_plan.get("ideal", 1) or 1), max(1, max_absolute()))
            sim_price = st.number_input(
                "Prezzo ipotizzato",
                min_value=1,
                step=1,
                value=max(1, sim_default),
                key="mossa_sim_price"
            )
            sc = mossa_scenario(sim_row, sim_price)

            if sc["status"].startswith("🟢"):
                st.success(f"{sc['status']} — dopo {sim_row.Nome} a {sim_price}")
            elif sc["status"].startswith("🟡"):
                st.warning(f"{sc['status']} — dopo {sim_row.Nome} a {sim_price}")
            else:
                st.error(f"{sc['status']} — dopo {sim_row.Nome} a {sim_price}")

            s1,s2,s3 = st.columns(3)
            s1.metric("Crediti dopo", sc["credits_after"])
            s2.metric("Giocatori mancanti", sc["missing_after"])
            s3.metric("Crediti/slot", sc["avg_per_slot"])

            st.caption(
                f"Riserva minima per chiudere: {sc['minimum_reserve']} • "
                f"Crediti liberi oltre la riserva: {sc['free_credits']} • "
                f"Piano residuo stimato: {sc['planned_remaining_total']}"
            )
            st.dataframe(sc["roles"], hide_index=True, use_container_width=True)
            st.info("La simulazione è di budget/strategia: non garantisce i futuri prezzi d'asta.")

    with st.expander("📊 MASTER CLASSICO", expanded=False):
        needed=[r for r in SLOTS if role_count(r)<SLOTS[r]]
        ap=all_players(); cand=ap[(~ap.key.isin(set(S["out"])))&ap.Ruolo.isin(needed)].copy()
        if len(cand):
            cand["Fascia"]=cand.FVM.map(tier)
            cand["STOP"]=cand.apply(cap_for,axis=1)
            cand["Previsione"]=cand.apply(lambda r:f"{prediction(r)[0]}-{prediction(r)[1]}",axis=1)
            cand["Fit"]=cand.apply(fit,axis=1)
            st.dataframe(
                cand.sort_values(["Fit","FVM"],ascending=False)[["Nome","Ruolo","Squadra","FVM","Fascia","Previsione","STOP","Fit"]].head(15),
                hide_index=True,use_container_width=True
            )
        else:
            st.success("🏆 Rosa completa")

        st.markdown("#### Strategia di chiusura")
        rr=[]
        for r in SLOTS:
            missing=SLOTS[r]-role_count(r)
            rr.append({
                "Ruolo":r,"Presi":role_count(r),"Mancano":missing,"Urgenza":urgency(r),
...
def render_scommesse():
    st.subheader("🎲 Scommesse FC Jigen")
    st.caption("Low cost e profili da prendere solo al prezzo giusto. I valori IDEALE/STOP qui sono dedicati alla scommessa, non sostituiscono il Piano A/B/C.")
...
# Navigazione compatta: mostra una sola sezione alla volta, più leggera su iPhone.
main_nav = st.segmented_control(
    "Navigazione",
    ["🔥 Asta","🧠 Mossa","📋 Rosa","👥 Rivali","•••"],
    default="🔥 Asta",
    key="fm_main_nav",
    label_visibility="collapsed"
)
