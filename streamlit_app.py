# VERSIONE v3.21.4 FANTAMOSSA - FIX MOSSA + BOTTOM BAR
...
@st.fragment
def render_mossa():
    """Decision support stabile: una sola scelta ruolo, selezione per key reale."""
    fm_page("🧠 La Mossa", "Scegli un giocatore reale dell'asta: FantaMossa valuta convenienza, STOP e impatto sulla rosa.")
    ...
    if mossa_tool == "🎯 Compra":
        ...
        pool = available.copy()
        if m_role != "TUTTI":
            pool = pool[pool.Ruolo == m_role]
        pool = pool.sort_values(["FVM", "Nome"], ascending=[False, True])

        if pool.empty:
            st.success("🏆 Nessun giocatore disponibile nel ruolo selezionato.")
            return

        # La selectbox usa la KEY del giocatore, non il nome/indice:
        # elimina definitivamente gli incroci tipo Ramos -> Paz.
        player_map = {str(r.key): r for _, r in pool.iterrows()}
        player_keys = list(player_map.keys())

        picked_key = st.selectbox(
            "🔎 Cerca / scegli giocatore",
            player_keys,
            index=None,
            placeholder="Scrivi Lautaro, Nico Paz, Ramos, Dimarco...",
            key=f"mossa_buy_pick_{m_role}",
            format_func=lambda k: player_fast_label(player_map[str(k)])
        )
        ...
        if not picked_key:
            ...
            if not recs.empty:
                top = recs.iloc[0]
                st.markdown(
                    f'<div class="fm-pick"><div class="fm-pick-kicker">🔥 Suggerimento automatico • Score {int(top["Score"])}/100</div>'
                ...
                with st.expander("Vedi altre mosse consigliate", expanded=False):
                    st.dataframe(
                        recs[["Nome","R","Squadra","FVM","Score","Ideale","STOP","Stima"]],
                        hide_index=True,
                        use_container_width=True
                    )
            st.info("Seleziona un giocatore per avere il consiglio live sul suo prezzo.")
            return

        row = player_map[str(picked_key)]
        plan = effective_target_plan(row)
        info = player_intel(row)
        lo, hi = prediction(row)
        base_score = mossa_decision_score(row)

        st.markdown(
            f'<div class="fm-player-card">'
            f'<div class="fm-player-name">{row.Nome}</div>'
            f'<div class="fm-player-meta">{row.Ruolo} • {row.Squadra} • FVM {int(row.FVM)}</div>'
            f'</div>',
            unsafe_allow_html=True
        )

        k1, k2, k3 = st.columns(3)
        k1.metric("🎯 Priorità", str(plan.get("priority", "C")))
        k2.metric("💚 Ideale", int(plan.get("ideal", 1) or 1))
        k3.metric("🛑 STOP", int(plan.get("max", 1) or 1))

        st.caption(
            f"Score strategico base {base_score}/100 • Stima asta {lo}-{hi} • "
            f"Titolarità {info.get('titolarita','—')} • Rischio {info.get('risk','—')}"
        )

        default_price = max(1, min(int(plan.get("ideal", 1) or 1), max(1, max_absolute())))
        price = st.number_input(
            "💰 Prezzo / rilancio corrente",
            min_value=1,
            step=1,
            value=default_price,
            key=f"mossa_buy_price_{str(picked_key)}"
        )

        sig = auction_signal(row, int(price), plan, info)
        score_now = mossa_decision_score(row, int(price))
        label = str(sig.get("label", ""))
        effective_stop = int(sig.get("effective_stop", plan.get("max", 1)) or 1)

        if label.startswith("🟢"):
            st.success(f"{label} — Score {score_now}/100")
        elif label.startswith("🟡"):
            st.warning(f"{label} — Score {score_now}/100")
        else:
            st.error(f"{label or '🔴 LASCIA'} — Score {score_now}/100")

        st.progress(max(0, min(100, int(score_now))) / 100)

        sc = mossa_scenario(row, int(price))
        a, b, c = st.columns(3)
        a.metric("Crediti dopo", sc["credits_after"])
        b.metric("Slot mancanti", sc["missing_after"])
        c.metric("Crediti/slot", sc["avg_per_slot"])

        if int(price) > effective_stop:
            st.error(f"⛔ Sei oltre lo STOP operativo di {effective_stop}.")
        elif int(price) <= int(plan.get("ideal", 1) or 1):
            st.success(f"✅ Prezzo dentro l'IDEALE ({int(plan.get('ideal',1) or 1)}).")
        else:
            st.warning(f"⚠️ Sopra l'ideale ma ancora entro lo STOP operativo ({effective_stop}).")

        with st.expander("🧠 Perché FantaMossa lo valuta così", expanded=False):
            render_player_intel(row)
            st.write(
                f"**Urgenza ruolo:** {urgency(row.Ruolo)}/100  •  "
                f"**Scarsità:** {scarcity(row.Ruolo)}/100  •  "
                f"**Fit:** {fit(row)}/100"
            )
            pb = plan_b(row)
            if len(pb):
        ...
        ca = st.selectbox(
            "Giocatore A", cmp_keys, index=None, placeholder="Scegli A...",
            key="mossa_cmp_a_v2",
            format_func=lambda k: player_fast_label(cmp_map[str(k)])
        )
        cb = st.selectbox(
            "Giocatore B", cmp_keys, index=None, placeholder="Scegli B...",
            key="mossa_cmp_b_v2",
            format_func=lambda k: player_fast_label(cmp_map[str(k)])
        )

        if ca and cb:
            row_a = cmp_map[str(ca)]
            row_b = cmp_map[str(cb)]
            ...
            with cpa:
                pa = st.number_input("Prezzo A (0 = non noto)", min_value=0, step=1, value=0, key="mossa_price_a_v2")
            with cpb:
                pb = st.number_input("Prezzo B (0 = non noto)", min_value=0, step=1, value=0, key="mossa_price_b_v2")

            va, vb, winner = mossa_compare(row_a, row_b, pa, pb)
            if winner == "PARI":
                st.warning("⚖️ MOSSA: equilibrio quasi perfetto. Decidi sul prezzo reale.")
            else:
                st.success(f"🏆 MOSSA CONSIGLIATA: {winner}")
            st.dataframe(pd.DataFrame([va, vb]).set_index("Nome").T, use_container_width=True)

    else:
        av_sim = available.sort_values(["FVM","Nome"], ascending=[False,True]).copy()
        if av_sim.empty:
            st.info("Non ci sono giocatori disponibili da simulare.")
            return
        ...
        if sim_key:
            ...
            sim_price = st.number_input(
                ...
                key=f"mossa_sim_price_v2_{str(sim_key)}"
            )
