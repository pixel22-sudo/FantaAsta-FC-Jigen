...
@media (max-width:700px) {
  ...
  .st-key-quick_actions div[data-testid="stHorizontalBlock"] {
  ...
  .st-key-quick_actions div[data-testid="column"] {
    flex:1 1 calc(50% - .175rem) !important;
    width:auto !important;
    min-width:0 !important;
    max-width:calc(50% - .175rem) !important;
  }
  .st-key-quick_actions div.stButton,
  .st-key-quick_actions div.stButton > button {
    width:100% !important;
    max-width:100% !important;
  }
  .st-key-quick_actions div.stButton > button {
...
st.markdown(
    f"""<div class="jigen-head">
      <div class="jigen-brand">⚽ FC Jigen • v3.7.4 PRICES FIX</div>
...
with st.sidebar:
    ...
    if S["moves"] and st.button("↩️ ANNULLA",use_container_width=True):
...
with tabs[0]:
    ...
    with st.container(key="quick_actions"):
        ...
        with qa:
            st.button("☁️ SALVA",use_container_width=True,on_click=_quick_cloud_save)
        with qb:
            st.button("↩️ ANNULLA",use_container_width=True,on_click=_quick_undo,
