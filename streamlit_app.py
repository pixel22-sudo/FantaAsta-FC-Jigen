...
st.title("⚽ FantaAsta FC Jigen • WEB v3.3 STABILE iPHONE")
...
with tabs[0]:
    ...
    with left:
        q=st.text_input(
            "🔎 Cerca giocatore",
            placeholder="Scrivi nome o parte del nome, es. Lau",
        ...
        termine=q.strip()
        if termine:
            av=av[av.Nome.astype(str).str.contains(termine,case=False,na=False,regex=False)]
