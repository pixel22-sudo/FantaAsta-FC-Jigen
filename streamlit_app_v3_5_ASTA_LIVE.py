...
st.title("⚽ FantaAsta FC Jigen • WEB v3.5 ASTA LIVE")
st.caption("Ottimizzata per iPhone • Asta Live • Piano FC Jigen • Target editabili • Cloud • Backup")
...
tabs=st.tabs(["🔥 ASTA LIVE","🧠 ASTA MASTER","📡 RADAR","👥 AVVERSARI","📋 ROSA","🎯 PIANO ASTA","📈 STORICO"])
...
with tabs[0]:
    ...
    with left:
        q=st.text_input(
            "🔎 Cerca giocatore",
            placeholder="Es. Lautaro, Nico Paz, McTominay, Orsolini",
        ...
        if termine:
            if av.empty:
                st.warning(f"Nessun giocatore trovato per “{termine}”. Prova anche con il cognome.")
            else:
                alias_target = PLAYER_ALIASES.get(_norm_search(termine))
                if alias_target and alias_target in set(av["Nome"].astype(str)):
                    st.success(f"✅ Alias riconosciuto: {termine} → {alias_target}")
                st.caption(f"🔎 Trovati {len(av)} risultati • tocca il menu sotto per scegliere")
        labels=[f"{r.Nome} • {r.Ruolo} • {r.Squadra} • FVM {r.FVM}" for _,r in av.iterrows()]
        choice=st.selectbox(
            "Giocatore",
            labels,
            index=None,
            placeholder=("Nessun risultato" if not labels else "Tocca qui per scegliere..."),
        ...
        if choice:
            ...
            if plan:
                ...
                p1.metric("🎯 Priorità",plan.get("priority","C"))
                p2.metric("💚 Ideale",int(plan.get("ideal",0) or 0))
                p3.metric("🛑 STOP piano",int(plan.get("max",0) or 0))
                if plan.get("alternatives"):
                    st.info("Alternative: "+str(plan.get("alternatives")))
                if plan.get("notes"):
                    st.caption("Nota piano: "+str(plan.get("notes")))
